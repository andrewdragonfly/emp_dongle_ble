 ...
 */
 ...
 //writes will cause an iterruption on the other chip, and annother send will be the acutual command. If this is a direct command to the ble chip, then the interruption is not needed and will timeout

uint8_t log_request [5] = {0};

bool req_special_case;

void uart_request_send (){
        req_special_case = false;
                                     
        if (log_request[0] == 0xAD){
          internal_log_request_enable (); 
        }
        if(log_request[0] == 0xDF){
          req_special_case = true;
          send_dev_defines_uart(log_request[1],log_request[2]);
        }

        if(log_request[0] == 0xF7){
          send_dev_defines();
        }
        else{
        external_log_request_enable(log_request[0],log_request[1]);
        }
}
...
static void on_write(ble_nus_t * p_nus, ble_evt_t const * p_ble_evt)
{
             ...
             (p_nus->data_handler != NULL))
    {
        ...
        p_nus->data_handler(&evt);
           for (int i = 0; i<5; i++){
           log_request [i] = p_evt_write->data[i];
           }
...
uint32_t ble_nus_init(ble_nus_t * p_nus, ble_nus_init_t const * p_nus_init)
{
    ...
    // Add the TX Characteristic (everything else).
